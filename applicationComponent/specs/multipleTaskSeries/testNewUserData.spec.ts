...
describe('Add a multi series product and schedule a task for all the task series', () => {
    ...
    describe('Complete a single occurrence of each task series', async() => {
        ...
        beforeAll( () => {    
            ...
            specFileData.UserData  = __dataReader.loadJsonData('newUserData','multipleTaskSeries');
            ...
            try {
                ...
                specFileData.UserData.TaskSeries.forEach(async taskSeriesInfo => {
                    ...
                    taskSeriesInfo.taskScheduleInfo.forEach(async taskScheduleInfo => {
                        await Pages.cpSchedulerPage.ScheduleTaskWithObservations1(taskSeriesInfo.taskSeriesName, taskScheduleInfo);    
                    });
        ...
        it('Verify the specified count and status of the task occurrence with the scheduled task series', ()=> {
            try {
                ...
                specFileData.UserData.TaskSeries.forEach(taskSeriesInfo => {
                    ...
                    taskSeriesInfo.taskScheduleInfo.forEach(async taskScheduleInfo => {
                        let __expectedResult = Pages.cpSchedulerPage.calculateExpectedOccurrenceCountAndStatus(taskScheduleInfo.scheduleStartTime, taskScheduleInfo.scheduleEndTime, taskScheduleInfo.repeatEveryHour);
            ...
            try {
                ...
                specFileData.UserData.TaskSeries.forEach(taskSeriesInfo => {
                    ...
                    taskSeriesInfo.taskOccurrenceInfo.forEach(taskOccurrenceInfo => {
                        ...
                        Pages.cpSchedulerPage.updateOccurrenceDetailsWithObservations1(taskSeriesInfo.taskSeriesName, taskOccurrenceInfo);
                        browser.sleep(2000);
                    });
