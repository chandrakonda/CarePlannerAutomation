...
export class CarePlannerApiServices {
  ...
  parseResultOfMakePostRequest(response: any) { 
    ...
    let __result : Promise<any>;
    result = response;
    browser.logger.info("####Parse result of the response####");
    try {
        browser.logger.info(result);
        if (response.StatusCode >= 400) {
          browser.logger.info("Status code 400");
          __result = Promise.reject({ message: result });
        }
          ...
          if (result instanceof Object) {
            if (result.Data == null) {
              browser.logger.info("Instance of object result.data ==null");
              __result = Promise.resolve(result.StatusCode);
            } else {
              browser.logger.info("Instance of object result.data is not null");
              __result = Promise.resolve(result.Data);
            }
          } else {
            result = JSON.parse(result);
            if (result.Data == null) {
              browser.logger.info("result.data is null");
              __result = Promise.resolve(result.StatusCode);
            } else {
              browser.logger.info("result.data is not null");
              __result = Promise.resolve(result.Data);
            }
          }
        }
